import argparse
import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
from scipy import stats
from generator.config import (
    get_cached_model_color,
    get_size_in_billions,
    order_dict
)
import json

def load_human_baselines(questionnaire_file):
    """Load human baseline data from questionnaire file"""
    with open(questionnaire_file) as f:
        data = json.load(f)
        questionnaires = data['questionnaires']
        for q in questionnaires:
            if q['name'] == 'BFI':
                return {cat['cat_name']: cat['crowd'] for cat in q['categories']}
    return {}

def prepare_data(df):
    """Prepare and clean the data for plotting"""
    # Filter out random and specific models
    df = df[df['model_type'] != 'random']

    
    try:
        # Convert score to numeric, replacing invalid values with NaN
        df['score'] = pd.to_numeric(df['score'], errors='coerce')
        
        # Extract model sizes and convert to numeric
        df['model_size'] = df['model'].apply(lambda x: get_size_in_billions(x))
        
        # Add model family column
        df['model_family'] = df['model'].apply(lambda x: next(
            (family for family in ['llama', 'qwen', 'gemma','hist'] if family in x.lower()),
            'other'
        ))
        
        # Calculate variance and other statistics for each group
        variance_df = df.groupby(['model', 'model_family', 'model_size', 'persona', 'trait'])['score'].agg([
            'var',
            'count'
        ]).reset_index()
        
        # Calculate confidence intervals for variance using chi-square distribution
        variance_df['var_ci_lower'] = variance_df.apply(lambda row: 
            (row['count'] - 1) * row['var'] / stats.chi2.ppf(0.975, row['count'] - 1), axis=1)
        variance_df['var_ci_upper'] = variance_df.apply(lambda row: 
            (row['count'] - 1) * row['var'] / stats.chi2.ppf(0.025, row['count'] - 1), axis=1)
        
    except Exception as e:
        print(f"Error in data preparation: {e}")
        raise
    
    return variance_df


def plot_variance_scaling(df, output_path, human_baselines):
    """Create scaling plots showing variance trends per model family"""
    plt.rcParams['figure.figsize'] = [15, 10]
    plt.rcParams['figure.dpi'] = 300
    plt.rcParams['axes.grid'] = True
    plt.rcParams['grid.alpha'] = 0.2
    
    # Get unique traits and personas
    traits = sorted(df['trait'].unique())
    all_personas = order_dict['persona']
    personas = [p for p in all_personas if p in df['persona'].unique()]
    
    # Create figure and axes with adjusted spacing
    fig, axes = plt.subplots(len(personas), len(traits), 
                            figsize=(6*len(traits), 6*len(personas)),
                            squeeze=False)
    
    # Adjust subplot spacing
    plt.subplots_adjust(left=0.15, right=0.85, bottom=0.1, top=0.9,
                       wspace=0.3, hspace=0.2)
    
    # Color scheme for model families
    family_colors = {
        'llama': '#4169E1',
        'qwen': '#228B22',
        'gemma': '#8B0000',
        'hist': '#9932CC'    # Dark Orchid - adding hist model
    }
    
    # Set consistent y-axis limits
    y_min, y_max = 1e-2, 1e2  #BFI 1e-3, 1
    
    # Minor tick locations for log scale
    minor_ticks = [2,3,4,5,6,7,8,9,20,30,40,50,60,70,80,90,200,300,400]
    
    # Plot each subplot
    for i, persona in enumerate(personas):
        for j, trait in enumerate(traits):
            ax = axes[i, j]
            
            # Add thin border around subplot
            for spine in ax.spines.values():
                spine.set_linewidth(0.5)
            
            # Filter data for current persona and trait
            mask = (df['persona'] == persona) & (df['trait'] == trait)
            plot_data = df[mask].copy()
            
            try:
                # Plot points and trend lines for each family
                for family, color in family_colors.items():
                    family_data = plot_data[plot_data['model_family'] == family]
                    if not family_data.empty:
                        # Sort data by model size for line plotting
                        family_data = family_data.sort_values('model_size')
                        
                        # Plot confidence interval shading (more subtle)
                        ax.fill_between(family_data['model_size'],
                                      family_data['var_ci_lower'],
                                      family_data['var_ci_upper'],
                                      color=color, alpha=0.1)
                        
                        # Plot main line connecting points
                        ax.plot(family_data['model_size'], family_data['var'],
                               color=color, alpha=0.7, linewidth=2, zorder=3)
                        
                        # Plot individual points
                        ax.scatter(family_data['model_size'], family_data['var'],
                                 color=color, alpha=0.7, s=50, zorder=4)
                        
                        # Fit and plot trend line if enough points
                        if len(family_data) > 1:
                            z = np.polyfit(np.log10(family_data['model_size']),
                                         np.log10(family_data['var']), 1)
                            p = np.poly1d(z)
                            x_range = np.logspace(np.log10(family_data['model_size'].min()),
                                                np.log10(family_data['model_size'].max()),
                                                num=100)
                            ax.plot(x_range, 10**p(np.log10(x_range)),
                                  color=color, linestyle='--', alpha=0.3,
                                  label=f'{family.capitalize()}')
                
                # Set scales and limits
                ax.set_xscale('log')
                ax.set_yscale('log')
                ax.set_xlim(1, 500)
                ax.set_ylim(y_min, y_max)
                
                # Set ticks
                ax.set_xticks([1, 10, 100])
                ax.set_xticks(minor_ticks, minor=True)
                ax.tick_params(which='minor', length=2)
                ax.tick_params(which='major', length=4)
                
                # Format axis labels
                if i == len(personas)-1:
                    ax.set_xlabel('Model Size (B)', fontsize=12)
                if j == 0:
                    ax.set_ylabel('Response Variance', fontsize=12)
                
                # Set title for top row only
                if i == 0:
                    ax.set_title(trait, fontsize=20, pad=15, fontweight='bold')
                
                # Add grid
                ax.grid(True, which='both', alpha=0.2)
                
            except Exception as e:
                print(f"Error plotting for persona {persona}, trait {trait}: {e}")
                continue
    
        # Add persona labels
        ax = axes[i, 0]
        ax.text(-0.3, 0.5, persona,
                transform=ax.transAxes,
                fontsize=20,
                va='center',
                ha='right',
                rotation=90,
                fontweight='bold')
    
    # Add legend
    handles, labels = axes[0, 0].get_legend_handles_labels()
    fig.legend(handles, labels,
              loc='center right',
              bbox_to_anchor=(0.95, 0.5),
              title="Model Families",
              title_fontsize=14,
              fontsize=14,
              frameon=True,
              edgecolor='black',
              borderpad=1)
    
    # Add overall title
    fig.suptitle('Response Variance Scaling Analysis\nLower values indicate more stable responses',
                 fontsize=24, y=0.95)
    
    # Save plots
    for fmt in ['svg', 'png']:
        save_path = f'{output_path}/variance_scaling.{fmt}'
        plt.savefig(save_path, format=fmt, bbox_inches='tight', dpi=300)
        print(f"Saved plot as {save_path}")
    
    plt.close()

def main():
    parser = argparse.ArgumentParser(description="Create variance scaling behavior plots")
    parser.add_argument("input_file", help="Path to input CSV file")
    parser.add_argument("output_dir", help="Path to output directory")
    parser.add_argument("--questionnaire", default="questionnaires.json",
                       help="Path to questionnaire JSON file")
    args = parser.parse_args()
    
    try:
        # Load human baselines
        human_baselines = load_human_baselines(args.questionnaire)
        
        # Read and prepare data
        print("Reading data...")
        df = pd.read_csv(args.input_file)
        
        print("Preparing data...")
        variance_df = prepare_data(df)
        
        # Create plots
        print("Creating variance scaling plots...")
        plot_variance_scaling(variance_df, args.output_dir, human_baselines)
        print("Plotting complete")
        
    except Exception as e:
        print(f"Error in main execution: {e}")
        raise

if __name__ == "__main__":
    main()